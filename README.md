...
stone_engineはまだ始まったばかりのレンダリングエンジンであり、まだその目的への道程もはっきりとしていない。
ライブラリとしては、モジュール化やパッケージ化などが求められるか、現時点ではそれは時期尚早と考える。

モジュール化を行うということは、その外部に対するAPIを定義するということであり、
機能と使い方を明確化することである。
stone_engineは、これがまだその段階に至っていないと自覚している。

当面の間、モジュール化は行わず、プログラマが直でそれぞれのクラス内部にアクセスしたり、サブクラス化を行うのを、
推奨する立場である。
